...
if __name__ == "__main__":
    ...
    message = sys.argv[2] if len(sys.argv) > 2 else "gpu"
    
    print("="*50)
    print("GPU Mode - ROS Publisher")
    print("="*50)
    
    success = publish_ros_message(topic, message)
    
    if success:
        print("[SUCCESS] GPU mode set")
        print("="*50)
        sys.exit(0)
    else:
        print("[ERROR] Failed to set GPU mode")
        sys.exit(1)
